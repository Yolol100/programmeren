...
<?php
/** Local CI provider mock. Loaded only in the disposable audit runtime. */
add_filter('pre_http_request', static function ($pre, $args, $url) {
    if (false === strpos((string) $url, 'api.cloudflare.com/client/v4/')) {
        return $pre;
    }
    $mode = (string) get_option('programmeren_provider_mode', 'success');
    if ('timeout' === $mode) {
        return new WP_Error('http_request_failed', 'programmeren simulated timeout');
    }
    $code = '429' === $mode ? 429 : ('500' === $mode ? 500 : 200);
    $body = 'oversize' === $mode
        ? str_repeat('x', 70000)
        : wp_json_encode(array('success' => 200 === $code, 'mode' => $mode));
    return array(
        'headers'  => array('content-type' => 'application/json'),
        'body'     => $body,
        'response' => array('code' => $code, 'message' => 200 === $code ? 'OK' : 'Simulated failure'),
        'cookies'  => array(),
        'filename' => null,
    );
}, 10, 3);
...
run_check "required-extensions" php -r '
    $required = ["mysqli", "redis", "apcu"];
    $missing = array_values(array_filter($required, fn($e) => !extension_loaded($e)));
    if ($missing) { fwrite(STDERR, "missing=" . implode(",", $missing) . "\n"); exit(1); }
    if (!filter_var(ini_get("apc.enabled"), FILTER_VALIDATE_BOOLEAN)) { fwrite(STDERR, "APCu disabled\n"); exit(1); }
    echo "extensions=ok\n";
'
...
if (( FAILURES > 0 )); then
    echo "Required runtime services unavailable; integration runtime cannot continue." >&2
    exit 1
fi
...
{
    wp_cmd core version | sed 's/^/wordpress=/'
    wp_cmd plugin get audit-target --field=version | sed 's/^/target_plugin=/'
    wp_cmd plugin get woocommerce --field=version | sed 's/^/woocommerce=/'
    php -r 'echo "redis_ext=" . phpversion("redis") . PHP_EOL; echo "apcu_ext=" . phpversion("apcu") . PHP_EOL;'
} > "$RESULTS/integration-runtime-versions.txt" 2>&1
...
PRODUCT_CODE=$?
set -e
if [[ $PRODUCT_CODE -eq 0 && "$PRODUCT_ID" =~ ^[0-9]+$ && "$PRODUCT_ID" -gt 0 ]]; then
    echo "product_id=$PRODUCT_ID" > "$RESULTS/integration-woocommerce-product.log"
    record "woocommerce-product" PASS 0
else
    record "woocommerce-product" FAIL "${PRODUCT_CODE:-1}"
fi

if [[ -n "$PRODUCT_ID" && "$PRODUCT_ID" =~ ^[0-9]+$ ]]; then
    set +e
    curl -fsS -D "$RESULTS/integration-store-cart.headers" -o "$RESULTS/integration-store-cart.json" "$BASE_URL/wp-json/wc/store/v1/cart"
    CART_CODE=$?
    set -e
    if [[ $CART_CODE -eq 0 ]]; then
        ...
        if [[ -n "$CART_TOKEN" ]]; then
            ...
            curl -fsS -X POST \
                ...
                -D "$RESULTS/integration-store-add-item.headers" \
                -o "$RESULTS/integration-store-add-item.json" \
            ...
            echo "Cart-Token header missing" > "$RESULTS/integration-store-add-item.log"
            record "woocommerce-store-add-item" FAIL 2
        fi
    else
        record "woocommerce-store-cart" FAIL "$CART_CODE"
    fi
fi
...
if [[ -n "$SERVER_PID" ]] && kill -0 "$SERVER_PID" 2>/dev/null; then
    set +e
    : > "$RESULTS/integration-concurrency.tsv"
    for path in / /wp-json/ /shop/ /cart/; do
        seq 1 12 | xargs -P 6 -I{} sh -c 'code=$(curl -sS -o /dev/null -w "%{http_code}" "$1" || printf 000); printf "%s\t%s\n" "$1" "$code"' _ "$BASE_URL$path" >> "$RESULTS/integration-concurrency.tsv"
    done
    BAD_HTTP="$(awk '$2 ~ /^5/ || $2 == "000" {count++} END {print count+0}' "$RESULTS/integration-concurrency.tsv")"
    set -e
...
if wp_cmd eval 'exit(class_exists("UCP_Object_Cache") ? 0 : 3);' >/dev/null 2>&1; then
    run_check "ucp-object-cache-capabilities" wp_cmd eval '
        $s=UCP_Object_Cache::status(true); echo wp_json_encode($s, JSON_PRETTY_PRINT)."\n";
        $ok=!empty($s["redis_connected"]) && !empty($s["apcu_available"]);
        exit($ok ? 0 : 1);
    '

    run_check "ucp-redis-dropin-install" wp_cmd eval '
        $m=new ReflectionMethod("UCP_Object_Cache", "write_dropin"); $m->setAccessible(true);
        $r=$m->invoke(null, "redis"); if (is_wp_error($r)) { fwrite(STDERR,$r->get_error_code()."\n"); exit(1); }
        exit($r === true ? 0 : 1);
    '
    run_check "ucp-redis-dropin-runtime" wp_cmd eval '
        if (!wp_using_ext_object_cache()) { fwrite(STDERR,"external object cache not active\n"); exit(1); }
        wp_cache_set("programmeren_probe", "redis-ok", "programmeren", 60);
        $v=wp_cache_get("programmeren_probe", "programmeren"); echo "value=".$v."\n"; exit("redis-ok" === $v ? 0 : 1);
    '
    run_check "ucp-redis-dropin-remove" wp_cmd eval '$r=UCP_Object_Cache::remove_owned_dropin(); exit(is_wp_error($r) ? 1 : 0);'

    run_check "ucp-apcu-dropin-install" wp_cmd eval '
        $m=new ReflectionMethod("UCP_Object_Cache", "write_dropin"); $m->setAccessible(true);
        $r=$m->invoke(null, "apcu"); if (is_wp_error($r)) { fwrite(STDERR,$r->get_error_code()."\n"); exit(1); }
        exit($r === true ? 0 : 1);
    '
    run_check "ucp-apcu-dropin-runtime" wp_cmd eval '
        if (!wp_using_ext_object_cache()) { fwrite(STDERR,"external object cache not active\n"); exit(1); }
        wp_cache_set("programmeren_probe", "apcu-ok", "programmeren", 60);
        $v=wp_cache_get("programmeren_probe", "programmeren"); echo "value=".$v."\n"; exit("apcu-ok" === $v ? 0 : 1);
    '
    ...
    if wp_cmd eval 'exit(class_exists("UCP_Runtime_Tests") ? 0 : 3);' >/dev/null 2>&1; then
        set +e
        wp_cmd eval '$r=UCP_Runtime_Tests::run_all(); echo wp_json_encode($r, JSON_PRETTY_PRINT)."\n"; foreach($r as $k=>$v){ if(is_array($v) && isset($v["status"]) && "fail" === $v["status"]) exit(1);} exit(0);' > "$RESULTS/integration-ucp-runtime-suite.json" 2> "$RESULTS/integration-ucp-runtime-suite.err"
        UCP_SUITE_CODE=$?
        set -e
    ...
    write_provider_mock
    run_check "ucp-provider-mock" wp_cmd eval '
        UCP_Options::update(array("cloudflare_zone_id"=>str_repeat("a",32),"cloudflare_api_token"=>"programmeren-audit-secret"));
        $cases=array("success"=>true,"429"=>false,"500"=>false,"timeout"=>false,"oversize"=>false);
        foreach($cases as $mode=>$expected){ update_option("programmeren_provider_mode",$mode,false); $got=UCP_Edge::cloudflare_purge_all(); echo $mode."=".($got?"true":"false")."\n"; if($got !== $expected) exit(1); }
        exit(0);
    '
    if grep -RIl --binary-files=without-match 'programmeren-audit-secret' "$WP_DIR/wp-content" "$RESULTS" 2>/dev/null | grep -v 'integration-ucp-provider-mock.log' > "$RESULTS/integration-secret-leak-files.txt"; then
        record "ucp-provider-secret-redaction" FAIL 1
    else
        record "ucp-provider-secret-redaction" PASS 0
    fi
    rm -f "$WP_DIR/wp-content/mu-plugins/programmeren-provider-mock.php"
fi

if [[ -n "$SERVER_PID" ]] && kill -0 "$SERVER_PID" 2>/dev/null; then
    mkdir -p "$RUNTIME_ROOT/browser"
    cp "${GITHUB_WORKSPACE:-$PWD}/.audit/runtime/browser-smoke.spec.js" "$RUNTIME_ROOT/browser/browser-smoke.spec.js"
    pushd "$RUNTIME_ROOT/browser" >/dev/null
    npm init -y >/dev/null 2>&1
    run_check "playwright-package" npm install --save-exact --no-audit --no-fund @playwright/test@1.62.1
    if grep -q $'^playwright-package\tPASS' "$STATUS"; then
        run_check "playwright-browser-install" npx playwright install chromium --with-deps
        BASE_URL="$BASE_URL" PLAYWRIGHT_SCREENSHOT="$RESULTS/integration-browser-home.png" run_check "browser-smoke" npx playwright test browser-smoke.spec.js --reporter=line --workers=1
    fi
    popd >/dev/null
fi
...
if [[ -f "$WP_DIR/wp-content/debug.log" ]]; then
    cp "$WP_DIR/wp-content/debug.log" "$RESULTS/integration-wp-debug.log"
else
    : > "$RESULTS/integration-wp-debug.log"
fi
if grep -Eiq 'PHP (Fatal error|Parse error)|Uncaught (Error|Exception)|Allowed memory size .* exhausted' "$RESULTS/integration-wp-debug.log" "$RESULTS/integration-php-server.log"; then
    record "runtime-fatal-scan" FAIL 1
else
    record "runtime-fatal-scan" PASS 0
fi
...
if (( FAILURES > 0 )); then
    echo "Ephemeral integration runtime completed with ${FAILURES} failing check(s)." >&2
    exit 1
fi
