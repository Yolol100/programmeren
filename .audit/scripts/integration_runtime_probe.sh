...
if wp_cmd eval 'exit(class_exists("UCP_Object_Cache")?0:3);' >/dev/null 2>&1; then
    ...
    run_check "ucp-redis-dropin-install" wp_cmd eval 'wp_set_current_user(1);$_SERVER["REQUEST_METHOD"]="POST";UCP_Options::update(array("enable_redis_object_cache"=>1));$_REQUEST["_wpnonce"]=wp_create_nonce("ucp_install_redis_object_cache");(new UCP_Object_Cache())->install_redis_dropin();'
    ...
    run_check "ucp-redis-dropin-remove" wp_cmd eval 'wp_set_current_user(1);$_SERVER["REQUEST_METHOD"]="POST";$_REQUEST["_wpnonce"]=wp_create_nonce("ucp_remove_object_cache_dropin");(new UCP_Object_Cache())->remove_object_cache_dropin();'
    run_check "ucp-apcu-dropin-install" wp_cmd eval 'wp_set_current_user(1);$_SERVER["REQUEST_METHOD"]="POST";UCP_Options::update(array("enable_apcu_object_cache"=>1));$_REQUEST["_wpnonce"]=wp_create_nonce("ucp_install_apcu_object_cache");(new UCP_Object_Cache())->install_apcu_dropin();'
    ...
    run_check "ucp-apcu-dropin-remove" wp_cmd eval 'wp_set_current_user(1);$_SERVER["REQUEST_METHOD"]="POST";$_REQUEST["_wpnonce"]=wp_create_nonce("ucp_remove_object_cache_dropin");(new UCP_Object_Cache())->remove_object_cache_dropin();'
